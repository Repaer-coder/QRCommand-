    ...
    <div className="shell">
      ...
      <main className="main">
        <div className="toprow">
          <div>
            <div className="eyebrow">Campaign control center</div>
            <h1>QR library</h1>
            <p className="muted">Search, pause, edit, download, and retire permanent campaigns.</p>
          </div>
        ...
        <div className="stats compactstats">
          ...
          <article className="card stat">
            <span>Active</span>
            <b>{(codes ?? []).filter((code) => code.status === 'active').length}</b>
          </article>
          <article className="card stat">
            <span>Paused</span>
            <b>{(codes ?? []).filter((code) => code.status === 'paused').length}</b>
          </article>
        </div>
        <form className="libraryfilters">
          <input className="input" name="q" defaultValue={params.q} placeholder="Search campaign names" />
        ...
        {!codes?.length ? (
          <section className="card empty">
            ...
            <p className="muted">Create a permanent campaign link. You can change its destination later without reprinting.</p>
            <Link className="btn" href="/dashboard/new">
